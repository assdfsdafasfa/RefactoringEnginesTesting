...
public class ConvertMoveMethodToExtract {
	// move -> rename, rename-.extract
	public void convertMoveMethodToExtract(IProject project, List<IFile> javaFiles, IFile testCaseFile,
			CompilationUnit cu, String fileName, String methodName, String targetClass)
							...
							if (node.getName().getIdentifier().equals(methodName)) {
								...
								cu.accept(new ASTVisitor() {
									...
									public boolean visit(TypeDeclaration node) {
										if (((TypeDeclaration) node).getName().toString().equals(targetClass)) {
											((TypeDeclaration) node).bodyDeclarations().add(methodDec);
											try {
...
//												System.out.println(modifiedSourceCode);
												SaveFile.saveContentToFile(
														Constant.CONVERT_PROGRAM_SAVE_PATH
																+ "convertMoveMethodToExtract/" + fileName + ".java",
														modifiedSourceCode);
...
//													System.out.println(modifiedSourceCode);
													SaveFile.saveContentToFile(Constant.CONVERT_PROGRAM_SAVE_PATH
															+ "convertMoveMethodToExtract/" + "Bug" + fileName
															+ ".java",
															modifiedSourceCode);
