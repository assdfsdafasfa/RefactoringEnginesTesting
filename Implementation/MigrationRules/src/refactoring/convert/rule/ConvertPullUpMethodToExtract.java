...
public class ConvertPullUpMethodToExtract {
	public void convertPullUpMethodToExtract(IProject project, List<IFile> javaFiles, IFile testCaseFile,
...
//								System.out.print("ast----------；" + ast + "\n");
								String parentClass = typeDeclaration.getSuperclassType().toString();// 父类名称

								ASTRewrite rewrite = ASTRewrite.create(ast);
								for (Object obj : cu.types()) {
									if (obj instanceof TypeDeclaration) {
										TypeDeclaration t = (TypeDeclaration) obj;
										if (t.getName().toString().equals(parentClass)) {
...
//												System.out.println(modifiedSourceCode);
												SaveFile.saveContentToFile(Constant.CONVERT_PROGRAM_SAVE_PATH
														+ "extractMethod/" + "Bug" + fileName + ".java",
														modifiedSourceCode);
						...
						public boolean visit(MethodDeclaration node) {
							if (node.getName().getIdentifier().equals(methodName)) {
								...
								for (String n : paraNameList) {// 为所有类型的声明，添加输出语句
									...
									Block methodBody = methodDec.getBody();
