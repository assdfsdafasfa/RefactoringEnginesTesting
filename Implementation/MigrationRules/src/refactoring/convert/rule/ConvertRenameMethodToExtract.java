...
public class ConvertRenameMethodToExtract {
	public void convertRenameMethodToExtract(IProject project, List<IFile> javaFiles, IFile testCaseFile,
			CompilationUnit cu, String fileName, String oldMethodName, String newMethodName)
			throws JavaModelException, IllegalArgumentException {
		MethodDeclaration methodDeclaration = FindIdentifier.findMethodDeclaration(cu, oldMethodName);
				...
				if (methodParaType.size() == 0) {
					typeDeclaration.accept(new ASTVisitor() {
						...
						public boolean visit(MethodDeclaration node) {
							if (node.getName().getIdentifier().equals(oldMethodName)) {
								...
								typeDeclaration.bodyDeclarations().add(methodDec);
								System.out.print("ast----------；" + ast + "\n");
								ASTRewrite rewrite = ASTRewrite.create(ast);
...
//									System.out.println(modifiedSourceCode);
									SaveFile.saveContentToFile(Constant.CONVERT_PROGRAM_SAVE_PATH
											+ "ConvertRenameMethodToExtract/" + "Bug" + fileName + ".java",
											modifiedSourceCode);
...
////											System.out.println(modifiedSourceCode);
//											SaveFile.saveContentToFile(Constant.CONVERT_PROGRAM_SAVE_PATH
//													+ "ConvertRenameMethodToExtract/" + fileName + ".java",
//													modifiedSourceCode);
//
//										} catch (BadLocationException e) {
//											e.printStackTrace();
...
//									System.out.println(modifiedSourceCode);
									SaveFile.saveContentToFile(Constant.CONVERT_PROGRAM_SAVE_PATH
											+ "ConvertRenameMethodToExtract/" + fileName + ".java", modifiedSourceCode);
