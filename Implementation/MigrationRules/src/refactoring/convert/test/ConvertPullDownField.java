...
import refactoring.convert.rule.ConvertPullDownFieldToMove;
import refactoring.file.parse.FindIdentifier;
import refactoring.file.parse.ParseFile;

public class ConvertPullDownField {
	public static void convertPullDownField(IProject project, List<IFile> javaFiles, IFile testCaseFile)
			throws JavaModelException, IllegalArgumentException {
		if (testCaseFile != null) {
			for (IFile javaFile : javaFiles) {// 遍历所有文件路径
				if (javaFile.toString().contains("PullDownField")) {// RenameMethod包所以文件
...
//						String newMethodNamme = arguments.get(1).replace("\"", "");
						...
						ConvertPullDownFieldToMove toMove = new ConvertPullDownFieldToMove();
						toMove.convertPullDownFieldToMove(project, javaFiles, testCaseFile, cu, fileName, fieldName);

//						ConvertPullDownFieldToRename toRename = new ConvertPullDownFieldToRename();
//						toRename.convertPullDownFieldToRename(project, javaFiles, testCaseFile, cu, fileName,
//								fieldName);
					}
				}
			}
